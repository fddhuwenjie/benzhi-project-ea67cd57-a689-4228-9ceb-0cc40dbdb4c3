...
func Open(path string) (*Store, error) {
	s := &Store{path: path, d: Data{Jobs: map[string]domain.RegistrationJob{}, Points: map[string][]domain.ControlPoint{}, Evals: map[string]domain.FitEvaluation{}, EvalHistory: map[string][]domain.FitEvaluation{}, Reviews: map[string]domain.Review{}, Remediations: map[string][]domain.Remediation{}, RemediationItems: map[string][]domain.RemediationItem{}, ReturnInstructions: map[string][]domain.ReturnInstruction{}, Manifests: map[string]domain.ReleaseManifest{}, Requests: map[string]Request{}}}
...
func clone(d Data) (Data, error) {
	...
	for id, request := range d.Requests {
		cloned := out.Requests[id]
		cloned.Response = request.Response
		out.Requests[id] = cloned
	}
	return out, e
}
