...
type replacementPreviewCacheKey struct {
	JobID    string
	Revision uint64
}
...
func (s *Service) PreviewReplacement(c ReplacePointCommand) (ReplacementPreview, error) {
	...
	key := replacementPreviewCacheKey{JobID: c.JobID, Revision: c.ExpectedRevision}
	if cached, ok := s.replacementPreviews.Load(key); ok {
		return cached.(ReplacementPreview), nil
	}
	preview, err := buildReplacementPreview(j, d.Points[c.JobID], d.Evals[c.JobID], c)
	if err != nil {
		return ReplacementPreview{}, err
	}
	s.replacementPreviews.Store(key, preview)
	return preview, nil
}
