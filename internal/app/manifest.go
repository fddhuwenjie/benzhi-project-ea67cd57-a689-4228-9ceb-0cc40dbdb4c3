...
var manifestVerificationCache sync.Map
...
func (s *Service) VerifyManifestReport(jobID string) (ManifestVerificationReport, error) {
	if cached, ok := manifestVerificationCache.Load(jobID); ok {
		if report, valid := cached.(ManifestVerificationReport); valid {
			return report, nil
		}
	}
	d := s.store.Snapshot()
	...
	manifestVerificationCache.Store(jobID, report)
	return report, nil
}
